    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
      ...
      >
        <div className="z-10 flex items-center gap-4 md:gap-5">
          <div className={`p-3 md:p-4 rounded-full transition-colors border ${activeMode === 'auto' ? 'bg-emerald-500 border-emerald-400 text-black' : 'bg-slate-700 border-slate-600 text-slate-400'}`}>
            <Zap className={`w-5 h-5 md:w-6 md:h-6 ${activeMode === 'auto' ? 'fill-current' : ''}`} />
          </div>
          <div>
            <h3 className={`text-lg md:text-xl font-bold tracking-wider ${activeMode === 'auto' ? 'text-emerald-400' : 'text-slate-400'}`}>
              AUTO-PILOT
            </h3>
            <p className="text-[10px] md:text-xs font-mono text-slate-500 mt-0.5 md:mt-1 uppercase tracking-widest">
              {activeMode === 'auto' ? 'AI Agent Active' : 'System Paused'}
            </p>
          </div>
        </div>
      ...
      >
        <div className="z-10 flex items-center gap-4 md:gap-5">
          <div className={`p-3 md:p-4 rounded-lg transition-colors border ${activeMode === 'manual' ? 'bg-indigo-500 border-indigo-400 text-white' : 'bg-slate-700 border-slate-600 text-slate-400'}`}>
            <Terminal className="w-5 h-5 md:w-6 md:h-6" />
          </div>
          <div>
            <h3 className={`text-lg md:text-xl font-bold tracking-wider ${activeMode === 'manual' ? 'text-indigo-400' : 'text-slate-400'}`}>
              MANUAL
            </h3>
            <p className="text-[10px] md:text-xs font-mono text-slate-500 mt-0.5 md:mt-1 uppercase tracking-widest">
              {activeMode === 'manual' ? 'Human Control' : 'Standby'}
            </p>
          </div>
        </div>
